...
import { getApiUrl, getCarritoUrl } from '../config/api.config';
...
cargarCarrito() {
  ...
  this.http.get<ItemCarrito[]>(getCarritoUrl(), { headers }).subscribe(
  ...
  agregarProducto(producto: Producto, cantidad: number = 1): Observable<any> {
    ...
    if (itemExistente) {
      ...
      return this.http.patch(
        `${getApiUrl('/item-carrito')}/${itemExistente.id}/`,
  ...
  eliminarProducto(itemId: number): Observable<any> {
    const headers = this.getAuthHeaders();
    return this.http.delete(`${getApiUrl('/item-carrito')}/${itemId}/`, { headers })
      .pipe(tap(() => this.cargarCarrito()));
  }
